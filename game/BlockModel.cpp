...
BlockModel::BlockModel( int w, int h, int d, int level, const std::string &blockset, R250 &r):
    _width(w), _height(h), _depth(d), _level(level),
    _blockset(blockset), _r250(r),
    _orientation(0,0,1),
    _offset(0,0,d-1),
    _freefall(false),
    _freefallZ(0),
...
int BlockModel::getWidth( void){ return _width;}
int BlockModel::getHeight( void){ return _height;}
int BlockModel::getDepth( void){ return _depth;}
int BlockModel::getLevel( void){ return _level;}

int BlockModel::numBlocksInPlane( unsigned int plane)
{
    if( plane>=(unsigned int)_depth) return 0;
...
bool BlockModel::readBlock( ziStream &infile, int numElems, int &linecount)
{
    ...
    if( blockOK)
    {
        ...
        bi.center = Point3D( (maxPos.x + minPos.x)/2.0,
                             (maxPos.y + minPos.y)/2.0,
                             (maxPos.z + minPos.z)/2.0);
...
void BlockModel::updateHintList( void)
{
    ...
    for( ; i!=_elementListHint->end(); i++, j++)
    {
        ...
        while( (checkPos.z >= 0) &&
               (! elementLocked(checkPos)))
        {
            checkPos.z--;
        }

        checkPos.z++;
        if( checkPos.z > highest) highest = checkPos.z;
    ...
    else
    {
        ...
        if( now >= _nextDrop)
        {
            ...
            else
            {
                ...
                ScoreKeeperS::instance()->addToCurrentScore(
                    scoreToAdd, eCount, (int)GameState::stopwatch.getTime());
