...
public:
    ...
    std::string getCurrentScoreBoardName()
    {
    ...
    {
        return _leaderBoard[ _currentIndex];
    }
...
private:
    ...
    string leaderBoardToString( LeaderBoard &lb, 
                                const string &leaderBoardName);
    void stringToLeaderBoard( const std::string &lbString,
                              LeaderBoard &lb);
    ...
    hash_map< const std::string, time_t, hash<const std::string>, 
        equal_to<const std::string> > _lastOnlineRequestTime;
};
