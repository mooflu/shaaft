...
bool BlockView::init( void)
{
    ...
    GLBitmapCollection *blackBox =
        BitmapManagerS::instance()->getBitmap( "bitmaps/blackBox");
    if( !blackBox)
    {
    ...
    GLBitmapCollection *scoreBoard =
        BitmapManagerS::instance()->getBitmap( "bitmaps/scoreBoard");
    if( !blackBox)
    {
...
void BlockView::draw( void)
{
    ...
    if( (GameState::isDeveloper) || (GameState::context == Context::eInGame))
    {
        ...
        if( GameState::isAlive)
        {
            ...
            modelview = glm::rotate(modelview, glm::radians(getPrevAngle()),
                            glm::vec3(prevAxis.x, prevAxis.y, prevAxis.z));
...
void BlockView::drawIndicator( void)
{
    ...
    for( int i=0; i<_model.getDepth(); i++)
    {
        ...
        if( !count) _indicator->setColor( 0.2, 0.2, 0.2, 1.0 );
        else _indicator->setColor(getColor( i));
...
void BlockView::drawElement( Point3Di *p, BlockType blockType)
{
    ...
    else if( blockType == Hint)
    {
        ...
        modelview = glm::translate(modelview, glm::vec3(xp-halftilesize*hintSize, yp-halftilesize*hintSize, zp-halftilesize*hintSize));
