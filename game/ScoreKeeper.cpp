...
void ScoreKeeper::mergeOnlineScores()
{
    ...
    while( OnlineScore::GetNextScoreBoardData(boardName, scoreData))
    {
        ...
        if( _leaderBoardName == boardName)
        {
            ...
            {
                if(currentScore.score == _leaderBoard[i].score &&
                   currentScore.cubes == _leaderBoard[i].cubes &&
        ...
        else
        {
            ...
            if( boardName == _currentScoreboard->name)
            {
                stringToLeaderBoard(_currentScoreboard->data,
                                    _currentScoreboardAsLeaderBoard);
            }
        }
    }
}
...
void ScoreKeeper::mergeLeaderBoard( LeaderBoard &leaderBoard, const LeaderBoard &onlineBoard)
{
    ...
    for( oi=onlineBoard.begin(); oi!=onlineBoard.end(); oi++)
    {
        ...
        for( i=leaderBoard.begin(); i!=leaderBoard.end(); i++)
        {
            if(sd.score == i->score &&
               sd.cubes == i->cubes &&
               sd.secondsPlayed == i->secondsPlayed &&
               sd.time == i->time)
            {
                isDuplicate = true;
                i->online = true;
...
void ScoreKeeper::setLeaderBoard( const std::string &scoreboardName)
{
    ...
    {
        resetLeaderBoard( _leaderBoard);
        string data = leaderBoardToString( _leaderBoard,
                                           scoreboardName);
...
string ScoreKeeper::leaderBoardToString( LeaderBoard &lb,
                                         const string &leaderBoardName)
{
    ostringstream ostr;
...
void ScoreKeeper::stringToLeaderBoard( const string &lbString,
                                       LeaderBoard &lb)
{
...
const string ScoreKeeper::getInfoText( unsigned int index)
{
    ...
    if( index < _currentScoreboardAsLeaderBoard.size())
    {
        ...
        strftime( buf, 127,"%a %d-%b-%Y %H:%M",
            localtime(&_currentScoreboardAsLeaderBoard[index].time));
...
void ScoreKeeper::updateScoreBoardWithLeaderBoard( void)
{
    ...
    for( i=_scoreBoards.begin(); i<_scoreBoards.end(); i++)
    {
        ...
        {
            i->data = leaderBoardToString( _leaderBoard,
                                           _leaderBoardName);
            break;
        }
    }
...
void ScoreKeeper::setActive( string scoreboardName)
{
    ...
    else
    {
        ...
        string data = leaderBoardToString( _currentScoreboardAsLeaderBoard,
                                           scoreboardName);
        addScoreBoard( scoreboardName, data);
    }
...
void ScoreKeeper::draw( const Point2Di &offset)
{
    ...
    static GLBitmapCollection *icons =
    ...
    fontShadow->DrawString(
            scoreBoardHeader.c_str(),
            90+offset.x+9, 480+offset.y-9, 1.0f, 1.0f);
    fontWhite->DrawString(
            scoreBoardHeader.c_str(),
            90+offset.x, 480+offset.y, 1.0f, 1.0f);
    ...
    for( unsigned int i=0; i<maxIndex; i++)
    {
        ...
        fontShadow->DrawString(
            place,
            80+offset.x-placeWidth+7, 420+offset.y-(float)i*spacing-7, scale, scale);
        fontShadow->DrawString(
            _currentScoreboardAsLeaderBoard[ i].name.c_str(),
            90+offset.x+7, 420+offset.y-(float)i*spacing-7, scale, scale);
        fontShadow->DrawString(
            score,
            560+offset.x-scoreWidth+7, 420+offset.y-(float)i*spacing-7, scale, scale);
        ...
        fontWhite->DrawString(
            place,
            80+offset.x-placeWidth, 420+offset.y-(float)i*spacing, scale, scale);
        fontWhite->DrawString(
            _currentScoreboardAsLeaderBoard[ i].name.c_str(),
            90+offset.x, 420+offset.y-(float)i*spacing, scale, scale);
        fontWhite->DrawString(
            score,
            560+offset.x-scoreWidth, 420+offset.y-(float)i*spacing, scale, scale);
    }
}
