...
vector<Complex> Polynomial::Solve( double a, double b, double c, double d, double e)
{
    ...
    vector<Complex> cubicSolution =
        Solve( 1, f/2, (f*f - 4*h)/16, -g*g/64 );
    ...
    else
    {
        ...
        double p[3];
        int idx=0;
        if( y1 > 0) p[idx++] = sqrt(y1);
        if( y2 > 0) p[idx++] = sqrt(y2);
        if( y3 > 0) p[idx++] = sqrt(y3);
...
**/
bool fxShift(int NN, int L2, const vector<Complex> &P, vector<Complex> &QP, vector<Complex> &H, vector<Complex> &QH, vector<Complex> &SH)
{
    bool conv = false;
    int N = NN - 1;
